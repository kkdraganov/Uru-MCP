...
class IntelligentToolLoader {
    ...
    async getDiscoveryTools(apiKey = null) {
        ...
        try {
            ...
            // Fallback to app-based namespace discovery when the richer endpoint is unavailable.

            try {

                this.log(`ðŸ”„ Falling back to app discovery`);
                ...
                // Emergency fallback - return no discovery tools when backend discovery is unavailable.
                // Surfacing stale or guessed namespaces in this state is misleading.

                this.log(`ðŸš¨ Using empty emergency fallback for discovery tools`);
     ...
     * Emergency fallback discovery tools
     *
     * Return an empty list rather than stale or guessed namespaces when
     * namespace discovery and app discovery are both unavailable.
