...
async function main() {
    try {
        ...
        if (!config.token) {
            ...
            console.error(
                chalk.gray(
                    '   To configure a default API key: Use --key or run --setup'
                )
            );
        }
...
async function runSetupWizard() {
    ...
    console.log(
        '2. Add the server to Claude Desktop (run: npx uru-mcp --claude-config)'
    );
}
...
function showClaudeConfig() {
    ...
    console.log(
        'For VS Code, Cursor, or other MCP clients, use similar configuration:\n'
    );
