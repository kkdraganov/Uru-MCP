...
class DynamicToolRegistry {
  ...
  registerNamespaceTools(namespace, tools) {
    const namespacedTools = tools.map(tool => ({
      ...tool,
      name: `${namespace}_${tool.originalName || tool.name}`,
  ...
  async fetchNamespacesFromProxy(apiKey = null) {
    try {
      ...
      const namespacesResponse = await axios.get(`${this.proxyUrl}/namespaces`, {
        timeout: 3000, // Reduced timeout for Claude Desktop compatibility
        headers: this.getAuthHeaders(apiKey)
      });

      this.log(`📊 Namespaces response: ${JSON.stringify(namespacesResponse.data, null, 2)}`);

      if (!Array.isArray(namespacesResponse.data.namespaces)) {
        throw new Error('Invalid namespaces response from proxy - expected array in namespaces property');
      }
      ...
      this.log(`✅ Loaded ${namespaces.length} namespaces with connection metadata`);
      return namespaces;
    } catch (error) {
      this.log(`❌ Error fetching namespaces from proxy: ${error.message}`, 'error');
      ...
      if (error.response?.status === 401) {
        throw new Error('Authentication failed when fetching namespaces. Check your API key.');
      } else if (error.response?.status === 404) {
        throw new Error('Namespaces endpoint not found. The MCP proxy may not support enhanced namespace discovery.');
      } else if (error.code === 'ECONNREFUSED') {
        throw new Error(`Cannot connect to MCP proxy at ${this.proxyUrl}. Check if the proxy is running.`);
      } else {
  ...
  async fetchAppsFromProxy(apiKey = null) {
    try {
      ...
      const appsResponse = await axios.get(`${this.proxyUrl}/list/apps`, {
        timeout: 2000, // Reduced timeout for Claude Desktop compatibility
        headers: this.getAuthHeaders(apiKey)
      });
      ...
      const appNames = appsResponse.data.map(app => {
        if (typeof app === 'string') {
          ...
          throw new Error(`Invalid app format: expected string or object with name property, got ${typeof app}`);
        }
      });
  ...
  async fetchToolsForApp(appName, apiKey = null) {
    try {
      ...
      if (Array.isArray(toolsResponse.data)) {
        ...
        throw new Error(`Invalid tools response for app '${appName}' - expected array or object with tools array`);
      }
