...
class ToolNamespaceManager {
    ...
    async fetchNamespacesFromProxy(apiKey = null, options = {}) {
        const {
            ...
            minVersion = null,
            maxReadyRetries = 3,
            readyRetryBaseMs = 500,
        } = options || {};
        const parsedMinVersion = Number(minVersion);
        const effectiveMinVersion = Number.isFinite(parsedMinVersion)
            ? parsedMinVersion
            : Number(this.requiredMinVersion || 0);
        const shouldUseCache = !forceRefresh && effectiveMinVersion <= 0;
        ...
        while (true) {
            try {
                ...
                if (shouldUseCache && cachedNamespaces) {
                    this.log(`�o. Returning ${cachedNamespaces.length} cached namespaces`);
                    return cachedNamespaces;
                }

                /** @type {{ timeout: number, headers: Record<string, string>, params?: Record<string, number> }} */
                const requestConfig = {
                    timeout: this.config.timeout || 30000,
                    headers: this.getAuthHeaders(apiKey),
                };
                if (effectiveMinVersion > 0) {
                    requestConfig.params = { min_version: effectiveMinVersion };
                }
                const namespacesResponse = await axios.get(
                    `${this.proxyUrl}/namespaces`,
                    requestConfig
                );
                ...
                // Retry bounded times when proxy explicitly reports "tools not ready".
                if (error.response?.status === 409 && effectiveMinVersion > 0) {
                    const retryAfterMs = Number(
                        error.response?.data?.retry_after_ms ||
                            readyRetryBaseMs * Math.pow(2, readyAttempt)
                    );
                    if (readyAttempt < maxReadyRetries) {
                        const waitMs = Math.min(
                            Math.max(retryAfterMs, 200),
                            5000
                        );
                        readyAttempt += 1;
                        this.log(
                            `⏳ Tools not ready for min_version=${effectiveMinVersion}; retry ${readyAttempt}/${maxReadyRetries} in ${waitMs}ms`,
                            'warn'
                        );
                    ...
                    throw new Error(
                        `Tools not ready for required version ${effectiveMinVersion} after ${maxReadyRetries} retries`
                    );
                }
    ...
    /**
     * Require upcoming namespace fetches to wait for at least this tools version.
     * @param {number} version
     */
    setRequiredMinVersion(version) {
        const parsed = Number(version);
        if (!Number.isFinite(parsed) || parsed <= 0) {
            this.requiredMinVersion = 0;
            return;
        }
        this.requiredMinVersion = parsed;
    }
