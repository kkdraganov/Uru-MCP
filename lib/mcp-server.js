    ...
    if (tools.length > 0) {
      ...
      if (exampleTool.inputSchema && exampleTool.inputSchema.properties) {
        ...
        for (const [paramName, paramDef] of Object.entries(properties)) {
          ...
          const isRequired = required.includes(paramName);
          if (isRequired || paramCount < 2) { // Show required params and up to 2 optional ones
            let exampleValue;
            ...
            response += `    "${paramName}": ${exampleValue}${paramCount < Math.min(Object.keys(properties).length - 1, 2) ? ',' : ''}\n`;
            paramCount++;
          }
        }
      } else {
