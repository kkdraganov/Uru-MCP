...
} = require('./connection-diagnostics');
...
This hierarchical approach provides full MCP compliance while efficiently managing large tool catalogs.`,
            },
            {
                capabilities: {
                    tools: {
                        listChanged: config.enableToolListChanged !== false,
                    },
                    resources: {},
                    prompts: {},
                    logging: {
    ...
    async start() {
        try {
            ...
            this._bindStdioLifecycle(process.stdin);
    ...
    /**
     * Validate configured authentication, or only connectivity when API keys are
    ...
    async testProxyConnection() {
        if (this.token) {
            ...
            this.log('✅ Authenticated namespace and workspace connection successful');
            return true;
        }
        ...
        this.log('✅ Proxy connectivity successful via /health');
        return true;
    }
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async request => {
            try {
                ...
                const rawToolArgs = request.params.arguments;
                let apiKey = null;
                let cleanedArgs = rawToolArgs;
                let debugArgs = rawToolArgs;

                if (rawToolArgs && typeof rawToolArgs === 'object') {
                    cleanedArgs = { ...rawToolArgs };
                    debugArgs = { ...rawToolArgs };
                    if (
                        ...
                        apiKey = rawToolArgs.api_key || null;
                        delete cleanedArgs.api_key;
                        debugArgs.api_key = '[REDACTED]';
                    }
                }
                ...
                this.log(`📝 Arguments: ${JSON.stringify(debugArgs, null, 2)}`);
    ...
    async executeWrapperToolOnProxy(
        ...
        try {
            ...
            return this.formatProxyExecuteResponse(response.data, wrapperToolName);
        } catch (error) {
            this.log(
    ...
    formatProxyExecuteResponse(responseData, toolName) {
    ...
    startToolsVersionMonitor() {
        try {
            if (this.config.enableToolListChanged === false) {
                this.log('[INFO] Live tools/list_changed notifications disabled');
                return;
            }
            ...
            if (typeof this._toolsVersionMonitor.unref === 'function') {
                this._toolsVersionMonitor.unref();
            }
            this.log(
    ...
    _bindStdioLifecycle(stdin) {
        if (!stdin || this._stdinLifecycleHandlers) {
            return;
        }

        const handleStreamClosure = eventName => {
            this.log(`[INFO] Detected stdin ${eventName}; shutting down MCP server`);
            void this.shutdown(`stdin_${eventName}`).finally(() => {
                process.exit(0);
            });
        };

        const onEnd = () => handleStreamClosure('end');
        const onClose = () => handleStreamClosure('close');

        stdin.once('end', onEnd);
        stdin.once('close', onClose);

        this._stdinLifecycleHandlers = {
            stdin,
            onEnd,
            onClose,
        };
    ...
    _unbindStdioLifecycle() {
        if (!this._stdinLifecycleHandlers) {
            return;
        }

        const { stdin, onEnd, onClose } = this._stdinLifecycleHandlers;
        stdin.off('end', onEnd);
    ...
    async shutdown(reason = 'shutdown') {
        ...
        this._shutdownPromise = (async () => {
            this.log(`[INFO] Shutting down MCP server (${reason})`);
            this._unbindStdioLifecycle();
            ...
            try {
                ...
                this.log(
                    `[WARN] Error while closing MCP server transport: ${error.message}`,
                    'warn'
                );
            }
            ...
            this._transport = null;
            this.isConnected = false;
            this.log('[INFO] MCP server shutdown complete');
        })();
