...
1. DISCOVERY: Call tools/list to see namespace discovery tools (e.g., gmail_work_kal_list_tools, platform_list_tools)

2. EXPLORATION: Call namespace discovery tools (e.g., gmail_work_kal_list_tools) to load and explore tools in that namespace

3. EXECUTION: Call specific namespaced tools directly (e.g., gmail_work_kal_send_email, platform_manage_users)

EXAMPLE WORKFLOW:
- Call tools/list → See [gmail_work_kal_list_tools, platform_list_tools, ...]
- Call gmail_work_kal_list_tools → Loads Gmail tools and shows available options
- Call gmail_work_kal_send_email with parameters → Email gets sent
...
1. **DISCOVERY**: You see namespace discovery tools like \`gmail_work_kal_list_tools\`, \`platform_list_tools\`, etc.
...
**Example Workflow:**
\`\`\`
1. Call gmail_work_kal_list_tools → Loads Gmail tools and shows available options
2. Call gmail_work_kal_send_email with email parameters → Email gets sent
\`\`\`
...
**How to Use:**
1. Call namespace discovery tools (e.g., \`platform_list_tools\`)
...
**Error:** Could not fetch current namespaces: ${error.message}`
  ...
  async handleNamespaceDiscovery(toolName, args, apiKey = null) {
    const namespace = toolName.replace('_list_tools', '');
  ...
  formatToolDiscoveryResponse(appName, namespace, tools, args) {
    ...
    if (tools.length === 0) {
      ...
      response += `**Try:** Call \`${namespace}_list_tools\` without filters to see all available tools.`;
      return response;
    }
    ...
    response += `**🔍 Filter:** Use \`${namespace}_list_tools\` with filter parameter for specific tools`;
  ...
  async handleNamespacedToolExecution(toolName, args, apiKey) {
    ...
    if (!tool) {
      ...
      const [namespace] = toolName.split('_');
    ...
    if (!tool) {
      throw this.createMcpError(-32601, `Tool '${toolName}' not found`, {
        suggestion: `Use ${toolName.split('_')[0]}_list_tools to discover available tools`
      });
    }

    // Extract original tool name and app context
    const [namespace, ...toolParts] = toolName.split('_');
    const originalToolName = tool.originalName || toolParts.join('_');
    const appName = this.namespaceManager.denormalizeNamespace(namespace);
