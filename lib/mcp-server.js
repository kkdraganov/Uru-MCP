...
This hierarchical approach provides full MCP compliance while efficiently managing large tool catalogs.`,
    ...
    startToolChangeWatcher() {
        try {
            ...
            // Use a longer default interval to reduce namespace polling load in production
            const intervalMs = this.config?.cacheTimeout || 300000; // default 5 minutes
    ...
    async _getNamespacesSignature() {
        try {
            const namespaces = await this.namespaceManager.fetchNamespacesFromProxy(null, { forceRefresh: true });
