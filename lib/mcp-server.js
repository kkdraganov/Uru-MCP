  ...
  setupHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        ...
        // Handle help tool
        if (name === 'uru_help') {
          return await this.handleHelpTool();
        }

        // Handle namespace discovery tools
        if (name.endsWith('.list_tools')) {
          return await this.handleNamespaceDiscovery(name, cleanedArgs);
        }
...
**Error:** Could not fetch current namespaces: ${error.message}`
   ...
   */
  async handleNamespaceDiscovery(toolName, args) {
    ...
    try {
      ...
      const namespacedTools = await this.toolLoader.loadNamespace(namespace);
  ...
  async handleNamespacedToolExecution(toolName, args, apiKey) {
    ...
    this.log(`🔧 Executing tool '${originalToolName}' in app '${appName}'`);

    // Execute the tool via proxy
    return await this.executeToolOnProxy(originalToolName, args, appName, apiKey);
  }
      ...
      for (const appName of apps) {
        try {
          ...
          if (tool) {
            ...
            return await this.executeToolOnProxy(tool.originalName || toolName, args, appName, apiKey);
          }
  ...
  /**
   * Execute tool on proxy using REST API endpoint
   * @param {string} toolName - Name of the tool to execute
   * @param {object} parameters - Tool parameters
   * @param {string} appName - App name for context
   * @param {string} apiKey - Optional API key to use for this request
   */
  async executeToolOnProxy(toolName, parameters, appName, apiKey = null) {
    try {
      this.log(`🔧 Executing tool '${toolName}' in app '${appName}'`);
