...
1. DISCOVERY: Call tools/list to see namespace discovery tools (e.g., gmail_work_kal__list_tools, platform__list_tools)

2. EXPLORATION: Call namespace discovery tools (e.g., gmail_work_kal__list_tools) to load and explore tools in that namespace

3. EXECUTION: Call specific namespaced tools directly (e.g., gmail_work_kal__send_email, platform__manage_users)

EXAMPLE WORKFLOW:
- Call tools/list → See [gmail_work_kal__list_tools, platform__list_tools, ...]
- Call gmail_work_kal__list_tools → Loads Gmail tools and shows available options
- Call gmail_work_kal__send_email with parameters → Email gets sent
  ...
  setupHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        ...
        if (name.includes('__') && !name.endsWith('__list_tools')) {
...
1. **DISCOVERY**: You see namespace discovery tools like \`gmail_work_kal__list_tools\`, \`platform__list_tools\`, etc.
...
**Example Workflow:**
\`\`\`
1. Call gmail_work_kal__list_tools → Loads Gmail tools and shows available options
2. Call gmail_work_kal__send_email with email parameters → Email gets sent
\`\`\`
