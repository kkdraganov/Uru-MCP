...
This hierarchical approach provides full MCP compliance while efficiently managing large tool catalogs.`,
      ...
      if (!this.config.testMode) {
        try {
          ...
          try {
            ...
            this.log(`⚠️ Cache pre-warming failed: ${cacheError.message}`, 'warn');
          }
        } catch (error) {
          this.log(`⚠️ Proxy connection test failed: ${error.message}`, 'warn');
          this.log('⚠️ Server will start anyway - tools may fail until proxy is available', 'warn');
        }
      } else {
  ...
  async testProxyConnection() {
    ...
    throw new Error(`All proxy connection tests failed. Check if ${this.proxyUrl} is accessible.`);
  }
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        ...
        if (name.includes(':')) {
          ...
          name = toolName;
          this.log(`🔧 Stripped server prefix '${serverPrefix}:' from tool name`);
        }
        ...
        if (name.endsWith('__execute_tool')) {
          return await this.handleNamespaceExecuteTool(name, cleanedArgs, apiKey);
        }

        // Handle namespaced tool execution (legacy/backward compatibility)
        if (name.includes('__') && !name.endsWith('__list_tools') && !name.endsWith('__execute_tool')) {
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `# Uru Platform Two-Tool System

**How to Use:**

1. **DISCOVERY**: You see two tools per namespace: \`namespace__list_tools\` and \`namespace__execute_tool\`

2. **EXPLORATION**: Call \`namespace__list_tools\` to see available tools in that namespace

3. **EXECUTION**: Call \`namespace__execute_tool\` with tool_name and parameters

**Example Workflow:**
\`\`\`
1. Call gmail_work_kal__list_tools → Shows available Gmail tools
2. Call gmail_work_kal__execute_tool with:
   {
...
**Why This Design:**
This two-tool pattern ensures all tools are always available to Claude without dynamic loading issues, while efficiently managing 400+ tools.`
          }
        ]
      };
    } catch (error) {
...
**How to Use:**
1. Call \`namespace__list_tools\` to see available tools
2. Call \`namespace__execute_tool\` with tool_name and parameters
3. Each namespace has exactly two tools: list_tools and execute_tool
...
**Error:** Could not fetch current namespaces: ${error.message}`
  ...
  async handleNamespaceDiscovery(toolName, args, apiKey = null) {
    ...
    try {
      ...
      const availableTools = namespacedTools.filter(tool => !tool.name.endsWith('_list_tools'));
  ...
  formatToolDiscoveryResponse(appName, namespace, tools, args) {
    ...
    response += `**💡 Usage:** Use \`${namespace}__execute_tool\` with tool_name and parameters\n`;
    response += `**🔍 Filter:** Use \`${namespace}__list_tools\` with filter parameter for specific tools\n`;
    // Use the original tool name (without namespace prefix) in the example
    const exampleToolName = tools[0]?.originalName || tools[0]?.name || 'TOOL_NAME';
    response += `**📋 Example:** \`${namespace}__execute_tool\` with {"tool_name": "${exampleToolName}", "parameters": {...}}`;
  ...
  async handleNamespaceExecuteTool(toolName, args, apiKey = null) {
    ...
    this.log(`🚀 Executing tool via execute_tool: ${args.tool_name} in namespace: ${namespace} (${appName})`);

    // Validate required parameters
    if (!args.tool_name) {
      throw this.createMcpError(-32602, 'Missing required parameter: tool_name', {
        suggestion: 'Provide the tool_name parameter with the name of the tool to execute'
      });
    }
    ...
    this.log(`🔧 Executing tool '${targetToolName}' in app '${appName}' with namespace '${namespace}'`);

    try {
      // Execute the tool via proxy with namespace for connection routing
      const result = await this.executeToolOnProxy(targetToolName, toolParameters, appName, apiKey, namespace);

      // Debug logging to check the result format
      this.log(`🔍 Execute tool result: ${JSON.stringify(result, null, 2)}`, 'debug');
      ...
      if (!result || !result.content || !Array.isArray(result.content)) {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Tool execution completed but returned invalid format: ${JSON.stringify(result)}`
            }
          ]
        };
      }
      ...
      this.log(`❌ Failed to execute tool '${targetToolName}' in namespace '${namespace}': ${error.message}`, 'error');
      throw this.createMcpError(-32001, `Failed to execute tool '${targetToolName}'`, {
        error: error.message,
  ...
  async handleNamespacedToolExecution(toolName, args, apiKey) {
    ...
    if (!tool) {
      throw this.createMcpError(-32601, `Tool '${toolName}' not found`, {
        suggestion: `Use ${toolName.split('__')[0]}__list_tools to discover available tools`
      });
    }
    ...
    try {
      ...
      if (response.data && typeof response.data === 'object') {
        ...
        if (response.data.data !== undefined) {
          responseText = typeof response.data.data === 'string' ? response.data.data : JSON.stringify(response.data.data, null, 2);
        } else {
