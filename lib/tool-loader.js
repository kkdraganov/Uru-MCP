...
class IntelligentToolLoader {
  ...
  async getDiscoveryTools(apiKey = null) {
    try {
      // Use the new namespace discovery endpoint with aggressive timeout for Claude Desktop
      const namespaces = await Promise.race([
        this.namespaceManager.fetchNamespacesFromProxy(apiKey),
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Namespace fetch timeout')), 2000)
        )
      ]);
      ...
      // Fast fallback to legacy app-based discovery with shorter timeout
      try {
        this.log(`🔄 Falling back to legacy app discovery`);
        const apps = await Promise.race([
          this.namespaceManager.fetchAppsFromProxy(apiKey),
          new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Legacy fetch timeout')), 1000)
          )
        ]);
  ...
  getEmergencyDiscoveryTools() {
    ...
    return basicNamespaces.map(namespace =>
