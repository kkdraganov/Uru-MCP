  ...
  async getDiscoveryTools(apiKey = null) {
    try {
      ...
      for (const namespace of namespaces) {
        ...
        const fullDisplayName = `${displayName}${accountLabel}`;

        // Create both list_tools and execute_tool for each namespace
        tools.push(
          this.namespaceManager.createNamespaceDiscoveryTool(namespace.name, fullDisplayName),
          this.namespaceManager.createNamespaceExecuteTool(namespace.name, fullDisplayName)
        );
      }
      return tools;
    } catch (error) {
