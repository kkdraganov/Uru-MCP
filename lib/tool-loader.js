...
class IntelligentToolLoader {
    ...
    async getDiscoveryTools(apiKey = null) {
        try {
            ...
            for (const namespace of namespaces) {
                ...
                const label = (namespace.account_label || '').trim();
                if (!label) {
            ...
            try {
                ...
                this.log(
                    `❌ Fallback discovery also failed: ${fallbackError.message}`,
                    'error'
                );

                // Emergency fallback - return basic discovery tools for common namespaces
                this.log(`🚨 Using emergency fallback with basic discovery tools`);
    ...
    async preloadPriorityNamespaces() {
        ...
        this.log(
            `🔄 Pre-loading priority namespaces: ${this.preloadNamespaces.join(', ')}`
        );
        ...
        const loadPromises = this.preloadNamespaces.map(async namespaceName => {
            try {
                ...
                this.log(
                    `⚠️ Failed to pre-load namespace '${namespaceName}': ${error.message}`,
                    'warn'
                );
            }
        });
    ...
    async doLoadNamespace(namespaceName, apiKey = null) {
        ...
        try {
            ...
            const loadTime = Date.now() - startTime;
            this.log(
                `✅ Loaded namespace '${namespaceName}': ${registeredTools.length} tools in ${loadTime}ms`
            );

            return registeredTools;
        } catch (error) {
            this.log(
                `❌ Failed to load namespace '${namespaceName}': ${error.message}`,
                'error'
            );
            throw error;
        }
    ...
    async optimizeBasedOnUsage() {
        ...
        if (unloadedTopNamespaces.length > 0) {
            this.log(
                `🎯 Predictively loading top used namespaces: ${unloadedTopNamespaces.join(
                    ', '
                )}`
            );
            ...
            for (const namespace of unloadedTopNamespaces) {
                try {
                    ...
                    this.log(
                        `⚠️ Failed to predictively load '${namespace}': ${error.message}`,
                        'warn'
                    );
                }
            }
